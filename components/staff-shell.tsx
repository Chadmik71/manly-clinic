  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      <aside className="hidden sm:flex w-14 flex-col items-center border-r bg-[hsl(var(--rail))] py-3 gap-1 sticky top-0 self-start h-screen">
        <Link
          href="/staff"
          className="grid h-9 w-9 place-items-center rounded-md bg-primary text-primary-foreground font-bold mb-2"
          title={CLINIC.name}
        >
          M
        </Link>
        {RAIL.map((it) => {
          const active =
            pathname === it.href ||
            (it.href !== "/staff" && pathname.startsWith(it.href));
          const Icon = it.icon;
          return (
            <Link
              key={it.href}
              ...
              className={cn(
                ...
                active
                  ...
                  : "text-[hsl(var(--rail-foreground))] hover:bg-accent hover:text-accent-foreground",
              )}
            >
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="h-12 border-b bg-card flex items-center px-4 gap-4 text-sm sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-card/80">
          <span className="text-muted-foreground">
            {CLINIC.name} <span className="opacity-60">·</span>{" "}
            <span className="capitalize">
              {pathname.split("/")[2] ?? "today"}
            </span>
          </span>
...
/* Convenience date-nav widget for the topbar slot */
export function DateNav({
  ...
  const yest = new Date(day);
  yest.setDate(day.getDate() - 1);
  const tom = new Date(day);
  tom.setDate(day.getDate() + 1);
  const fmt = (d: Date) =>
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Link
        href={`${basePath}?date=${iso(new Date())}${q}`}
        className="ml-2 px-2 py-0.5 rounded-md border text-xs hover:bg-accent"
      >
        Today
      </Link>
    </div>
